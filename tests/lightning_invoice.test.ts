...
describe('Lightning invoice', () => {
	...
	it('should parse expire date', () => {
		const invoice = LightningInvoice.fromBolt11(goodInvoice)
		expect(invoice.expireDate?.toISOString()).toBe(
			new Date('2022-04-28T00:39:30.000Z').toISOString(),
		)
	})

	it('should parse creation date', () => {
		const invoice = LightningInvoice.fromBolt11(goodInvoice)
		expect(invoice.creationDate?.toISOString()).toBe(
			new Date('2022-04-28T00:29:30.000Z').toISOString(),
		)
	})
